...
#endif
#ifdef NAS_ENABLE
	debug("try to relocate FDT\n");
	ret = boot_relocate_fdt(lmb, of_flat_tree, &of_size);
	if (ret){
	    debug("FDT relocation failed, ret: %d\n", ret);
		return ret;
	}
#endif
...
static int rtk_plat_boot_prep_partition(void)
{
	...
	/* Use only one CPU in rescue mode */
	char *commandline = getenv("bootargs");
	char *tmp_cmdline = NULL;
	const char *cpu= "maxcpus=1";

	tmp_cmdline = (char *)malloc(strlen(commandline) + strlen(cpu));
	if (!tmp_cmdline) {
		printf("%s: Malloc failed\n", __func__);
	}
	else {
		sprintf(tmp_cmdline, "%s %s", commandline, cpu);
		setenv("bootargs", tmp_cmdline);		
		free(tmp_cmdline);
	}
	debug("Rescue mode:%s\n",getenv("bootargs"));
	...
	tmp_cmdline = (char *)malloc(strlen(commandline) + strlen(getenv("mtd_part")));
	if (!tmp_cmdline) {
		printf("%s: Malloc failed\n", __func__);
	}
	else {
...
static int rtk_plat_boot_prep_nas_partition(void)
{
	...
	tmp_cmdline = (char *)malloc(strlen(commandline) + strlen(nas_part));
	if (!tmp_cmdline) {
		printf("%s: Malloc failed\n", __func__);
	}
	else {
