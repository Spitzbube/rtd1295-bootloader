...
void board_init_f(ulong bootflag)
{
	...
	*((unsigned int *)&PRINTF_ADDRESS8) = (unsigned int)flush_cache;
	...
	{
		printf("[ERROR] TLB was overlaped with kernel. (addr: 0x%08x)\n", addr);
	}
...
/* used to save extern param, logo, rescue */
...
static int extern_param_from_SPI(void)
{
	...
	ext_para_ptr = (t_extern_param *) (*((unsigned int *)(BOARD_FLASH_ADDR_BASE +ARM_ROMCODE_SIZE + MIPS_RESETROM_SIZE)));
#else
...
//#ifdef CONFIG_SYS_RTK_NAND_FLASH
	...
	t_FLASH_read_descriptor flash_read;
	t_FLASH_ctrl_descriptor flash_ctrl;

	uint block_no, data_total_len;
	char fail_count;

	ext_para_ptr = (t_extern_param *) EXTERN_PARAM_FROM_FLASH_ADDR;

	/* copy ext_param, logo, and rescue from nand flash to ram */
	/* logo and rescue data minimal block number would be 5!!
	block 0: bad block table
	block 1: bad block table backup
	block 2: hw-setting
	block 3: hw-setting backup
	block 4: bootcode
	block 5: bootcode backup
	block 6: t_extern_param + logo + rescue
	block m: t_extern_param + logo + rescue backup
	block n: env param + magic num
	*/
	// copy t_extern_param data ( copy first 4K to insure that length is aligned to page boudnary)
	block_no = 6;
	flash_read.block_no = &block_no;
	flash_read.signature = NAND_BLOCK_DATA;
	flash_read.length = 0x1000;
	flash_read.buffer = (UINT8 *) EXTERN_PARAM_FROM_FLASH_ADDR;

	if( IO_read( SYS_MAJOR_FLASH, 0, &flash_read ) != OK )
	{
		printf("copy t_extern_param to ram fail!!\n");
		return -1;
	}
	...
	// calculate actual length need to copy
	data_total_len = sizeof(t_extern_param)
	...
	if (ext_para_ptr->logo5_img_len > 0)
		data_total_len += ext_para_ptr->logo5_img_len;
	if (ext_para_ptr->logo6_img_len > 0)
		data_total_len += ext_para_ptr->logo6_img_len;
	if (ext_para_ptr->logo7_img_len > 0)
		data_total_len += ext_para_ptr->logo7_img_len;
	if (ext_para_ptr->logo8_img_len > 0)
		data_total_len += ext_para_ptr->logo8_img_len;

	fail_count = 8;
	do
	{
		// copy ext_param, logo, and rescue from nand flash to memory
		flash_read.block_no = &block_no;
		flash_read.signature = NAND_BLOCK_DATA;
		flash_read.length = data_total_len;
		flash_read.buffer = (UINT8 *) EXTERN_PARAM_FROM_FLASH_ADDR;

		if( IO_read( SYS_MAJOR_FLASH, 0, &flash_read ) != OK )
		{
			if (*(flash_read.block_no) == -1)
			{
				// no more backup available
				printf("copy rescue and logo to ram fail!!\n");
				return -1;
			}
			else
			{
				// find next backup of ext_param, logo, and rescue
				block_no += data_total_len / fileflash_block_size + 1;
				fail_count--;
				continue;
			}
		}
...
// copy ENV param to DDR (already done in FLASH_NAND_install())
#ifndef Board_FACTORY_default
	memset((void *)&flash_ctrl, sizeof(flash_ctrl), 0);
	flash_ctrl.command = FLASH_CTRL_GET_ENV;
...
#if 0//#ifdef CONFIG_REALTEK_WATCHDOG
	WATCHDOG_KICK();
...
#if 0//#ifdef CONFIG_BSP_REALTEK
	puts("Extern param From: ");
