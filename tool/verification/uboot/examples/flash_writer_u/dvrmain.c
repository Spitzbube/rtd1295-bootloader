	...
	else if ( flash_magicno == FLASH_MAGICNO_NOR_SERIAL)
	{
	    ...
	    *((volatile unsigned int *)0x000000E0) = 0;
	    ...
	    if( bootcode2_boot_programmed_img_size ) {
		    ...
		    // update parameter
		    param.rescue_img_part0_saddr = (unsigned char *)spi_bootcode_addr2;
		    param.rescue_img_part0_len = bootcode2_boot_programmed_img_size;
		    
		    //rtprintf("\n");
		    //rtprintf("spi : rescue_img_part0_saddr=0x%08x\n", param.rescue_img_part0_saddr);
		    //rtprintf("spi : rescue_img_part0_len  =0x%08x\n", param.rescue_img_part0_len);

			// write parameter in bootcode2
		    //rtprintf("\nspi : write parameter2, start=0x%08x, len=0x%08x\n", spi_param_addr2, sizeof(t_extern_param));
		    //if ((*do_write)(device, (unsigned char *)&param, (unsigned int *)spi_param_addr2, sizeof(t_extern_param), 0, 0)!= 0 ) {
		    //    return -14;
		    //}
   		}
   		
   		if( bootcode3_boot_programmed_img_size ) {
			// write bootcode3	
			rtprintf("\nspi : write bootcode3, start=0x%08x, len=0x%08x\n", spi_bootcode_addr3, bootcode3_boot_programmed_img_size);
		    if ((*do_write)(device, bootcode3_boot_programmed_img_base, (unsigned int *)spi_bootcode_addr3, bootcode3_boot_programmed_img_size, 0, 0)!= 0 ) {
		        return -6;
		    }
	    
		    // update parameter
		    param.rescue_img_part1_saddr = (unsigned char *)spi_bootcode_addr3;
		    param.rescue_img_part1_len = bootcode3_boot_programmed_img_size;
		    
		    //rtprintf("\n");
		    //rtprintf("spi : rescue_img_part1_saddr=0x%08x\n", param.rescue_img_part1_saddr);
		    //rtprintf("spi : rescue_img_part1_len  =0x%08x\n", param.rescue_img_part1_len);

			// write parameter in bootcode3
		    //rtprintf("\nspi : write parameter3, start=0x%08x, len=0x%08x\n", spi_param_addr3, sizeof(t_extern_param));
		    //if ((*do_write)(device, (unsigned char *)&param, (unsigned int *)spi_param_addr3, sizeof(t_extern_param), 0, 0)!= 0 ) {
		    //    return -14;
		    //}
   		}
		#endif
