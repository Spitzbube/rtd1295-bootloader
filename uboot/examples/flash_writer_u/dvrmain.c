...
int dvrmain	( int argc, char * const argv[] )
{
    ...
    unsigned char * spi_bootcode64_addr;
...
//#endif
	...
	programmed_img64_base         = hasharray64;
    // fsbl
...
program_backup_copy_of_hwsetting:
	...
	#else
		#ifdef FOR_ICE_LOAD
			#ifdef NOT_ERASE_SPI_DATA_IN_FIRST_128KB
				prints("spi : erase 832KB from 0x");
				print_hex((SPI_CODE_PART1+0x20000));
				prints("\n");
			#else
				prints("spi : erase 1024KB from 0x");
				print_hex(SPI_CODE_PART1);
				prints("\n");
			#endif
        #endif
		#ifdef NOT_ERASE_SPI_DATA_IN_FIRST_128KB
			if ((*do_erase)(device, (unsigned int *)(SPI_CODE_PART1+0x20000), (0x00100000-0x20000)) !=0 ) {
				return -3;
			}	
		#else
