...
//#if defined(CONFIG_SYS_RTK_SATA_STORAGE)
//		return boot_flash_type=BOOT_SATA; // 3	
//#else
		return boot_flash_type=BOOT_NOR_SERIAL;	 // 1
//#endif
#elif defined(CONFIG_SYS_RTK_NAND_FLASH)
		rtd_outl(SYS_muxpad0,0x55555554);
		return boot_flash_type=BOOT_NAND; // 0
#else //emmc
...
char *pcb_get_boot_flash_type_string(void)
{
	...
	switch(flash_type) {
		case FLASH_TYPE_NAND: // 0
			return FLASH_TYPE_NAND_STRING;

		case FLASH_TYPE_SPI:  // 1
			return FLASH_TYPE_SPI_STRING;

		case FLASH_TYPE_EMMC: // 2
			return FLASH_TYPE_EMMC_STRING;

		case FLASH_TYPE_SATA: // 3
