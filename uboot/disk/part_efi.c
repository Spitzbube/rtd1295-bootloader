		...
		lbaint_t lastlba)
{
	...
	if (le64_to_cpu(gpt_h->first_usable_lba) > lastlba) {
		printf("GPT: first_usable_lba incorrect: 0x%08X%08X > " LBAF "\n",
		       (unsigned long)(le64_to_cpu(gpt_h->first_usable_lba)>>32),
		       (unsigned long)(le64_to_cpu(gpt_h->first_usable_lba)),
		       lastlba);
		return -1;
	}
	if (le64_to_cpu(gpt_h->last_usable_lba) > lastlba) {
		printf("GPT: last_usable_lba incorrect: 0x%08X%08X > " LBAF "\n",
		       (unsigned long)(le64_to_cpu(gpt_h->last_usable_lba)>>32),
		       (unsigned long)(le64_to_cpu(gpt_h->last_usable_lba)),
		       lastlba);
		return -1;
	}

	debug("GPT: first_usable_lba: 0x%08X%08X\n",
	      (unsigned long)(le64_to_cpu(gpt_h->first_usable_lba)>>32),
	      (unsigned long)(le64_to_cpu(gpt_h->first_usable_lba)));

	debug("GPT: last_usable_lba:  0x%08X%08X\n",
	      (unsigned long)(le64_to_cpu(gpt_h->last_usable_lba)>>32),
	      (unsigned long)(le64_to_cpu(gpt_h->last_usable_lba)));

	debug("GPT: last lba:         0x%08X%08X\n",
	      (unsigned long)(lastlba>>32),
...
gpt_entry* get_part_efi(block_dev_desc_t *dev_desc, u32 *numEntries)
{
	...
	debug("%s: gpt-entry at %p, numEntries=%d\n", __func__, gpt_pte, count);
...
void print_part_efi(block_dev_desc_t * dev_desc)
{
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {
		...
		printf("%3d\t0x%08x%08x\t0x%08lx%08x\t\"%s\"\n", (i + 1),
			(unsigned long)(le64_to_cpu(gpt_pte[i].starting_lba)>>32),
			(unsigned long)(le64_to_cpu(gpt_pte[i].starting_lba)),
			(unsigned long)(le64_to_cpu(gpt_pte[i].ending_lba)>>32),
			(unsigned long)(le64_to_cpu(gpt_pte[i].ending_lba)),
		...
		disk_partition_t *partitions, int parts_count)
{
	...
	if (ret) {
        printf("Error, gpt_fill_header failed.\n");
		goto err;
    }
	...
	if (ret) {
        printf("Error, gpt_fill_pte failed.\n");
		goto err;
    }
...
static int is_pmbr_valid(legacy_mbr * mbr)
{
	...
	if (!mbr || le16_to_cpu(mbr->signature) != MSDOS_MBR_SIGNATURE) {
        printf("Warning: failed to find the MSDOS MBR signature\n");
		return 0;
    }
    ...
    printf("Warning: failed to 0xEE type in MBR\n");
	return 0;
}
